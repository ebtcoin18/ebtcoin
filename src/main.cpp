...
uint256 hashGenesisBlock("0x000002affa0d8381a753d0ed7ebee5004fbd77bf1a37e428dfe7a0620c6fd96c");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EBTCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EBTCoin Signed Message:\n";
...
{
    // EBTCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // EBTCoin: each block
static const int64 nTargetSpacing = 3 * 60; // EBTCoin: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EBTCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, EBTCoin - DarkGravity, written by Evan Duffield - evan@EBTCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EBTCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EBTCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "EBTCoinX";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501608637;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3123579;
        ...
        assert(block.hashMerkleRoot == uint256("0xcbab620b46eb8444df21dbca19f461aaa840fb06dfa38d3271bec9c335d6075f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // EBTCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EBTCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EBTCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EBTCoinMiner(CWallet *pwallet)
{
    printf("EBTCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EBTCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running EBTCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EBTCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EBTCoinMiner, pwallet));
}
